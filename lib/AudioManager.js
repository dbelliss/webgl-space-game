...
class AudioManager {
     ...
     */
    loadMusic() {
        var freeForm = new Audio("./Assets/freeForm.mp3");
    ...
    loadSounds() {
        ...
        for (var i = 0; i < 3; i++) {
            pickupObjectPool.push(new Audio("./Assets/pickup.ogg"));
        }
        this.sounds[SoundsEnum.PICKUP] = pickupObjectPool;

        var crashSoundPool = []
        for (var i = 0; i < 2; i++) {
            crashSoundPool.push(new Audio("./Assets/crash.wav"));
        }
    ...
    playSong(songEnum) {
        if (this.music[songEnum] === undefined) {
            console.err("Could not find song ", song);
            return;
        }
        ...
        this._playAudio(this.currentSong, this.musicVolume);
    }
    ...
    playSound(soundEnum) {
        ...
        if (soundInstances === undefined) {
            console.err("Could not find sound ", soundEnum);
            return;
        }
        ...
        for (var i = 0; i < soundInstances.length; i++) {
            ...
            if (soundInstance.paused) {
                this._playAudio(soundInstance, this.sfxVolume)
                return
            }
        }
